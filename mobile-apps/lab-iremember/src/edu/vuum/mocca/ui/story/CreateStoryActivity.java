...
public class CreateStoryActivity extends StoryActivityBase {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		fragment = (CreateStoryFragment)
				getSupportFragmentManager().findFragmentById(android.R.id.content);

		if (savedInstanceState == null && null == fragment) {
	...
	private void launchSoundIntent() {
		...
		Intent soundRecordIntent = new Intent();
		soundRecordIntent.setClass(this, SoundRecordActivity.class);

		// Use getOutputMediaFile() to create a new
		// filename for this specific sound file
		String soundOutputFileName = getOutputMediaFile(MEDIA_TYPE_AUDIO).getAbsolutePath();

		// Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the SoundRecordActivity class, EXTRA_OUTPUT
		soundRecordIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT, soundOutputFileName);
	...
	private void launchCameraIntent() {
		...
		Uri imageOutputFileUri = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
		fragment.imagePath = imageOutputFileUri;

		// Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		imageIntent.putExtra(MediaStore.EXTRA_OUTPUT, imageOutputFileUri);
	...
	private void launchVideoCameraIntent() {
		...
		Uri videoOutputFileUri = getOutputMediaFileUri(MEDIA_TYPE_VIDEO);

		// Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		videoIntent.putExtra(MediaStore.EXTRA_OUTPUT, videoOutputFileUri);
